...
concept ContainPmrAlloc = requires() {
  ...
  std::is_constructible_v<typename T::allocator_type,
                          std::pmr::memory_resource*>;
};

template <typename T>
concept DefaultCtorWithAlloc =
    ContainPmrAlloc<T> &&
    requires(const typename T::allocator_type& alloc) { new T(alloc); };
...
void CmptTraits::UnsafeRegisterOne() {
  ...
  if constexpr (std::is_trivial_v<Cmpt>) RegisterTrivial(type.GetID());
  ...
  if constexpr (details::DefaultCtorWithAlloc<Cmpt>) {
    RegisterDefaultConstructor(
        type.GetID(), [](void* cmpt, std::pmr::memory_resource* world_rsrc) {
          using Alloc = typename Cmpt::allocator_type;
          Alloc alloc(world_rsrc);
          std::allocator_traits<Alloc>::template construct(
              alloc, reinterpret_cast<Cmpt*>(cmpt));
        });
  } else if constexpr (std::is_default_constructible_v<Cmpt>) {
    RegisterDefaultConstructor(
        type.GetID(),
        [](void* cmpt, std::pmr::memory_resource*) { new (cmpt) Cmpt(); });
  }

  if constexpr (std::is_destructible_v<Cmpt> &&
                !std::is_trivially_destructible_v<Cmpt>) {
    RegisterDestructor(type.GetID(),
                       [](void* cmpt) { static_cast<Cmpt*>(cmpt)->~Cmpt(); });
  }

  if constexpr (details::MoveCtorWithAlloc<Cmpt>) {
    RegisterMoveConstructor(
        type.GetID(),
  ...
  if constexpr (std::is_move_assignable_v<Cmpt> &&
                !std::is_trivially_move_assignable_v<Cmpt>) {
  ...
  if constexpr (details::CopyCtorWithAlloc<Cmpt>) {
    RegisterCopyConstructor(
        type.GetID(),
        [](void* dst, const void* src, std::pmr::memory_resource* world_rsrc) {
          using Alloc = typename Cmpt::allocator_type;
          Alloc alloc(world_rsrc);
          std::allocator_traits<Alloc>::template construct(
              alloc, reinterpret_cast<Cmpt*>(dst),
              *static_cast<const Cmpt*>(src));
        });
  } else if constexpr (std::is_copy_constructible_v<Cmpt> &&
                       !std::is_trivially_copy_constructible_v<Cmpt>) {
    RegisterCopyConstructor(type.GetID(), [](void* dst, const void* src,
                                             std::pmr::memory_resource*) {
...
void CmptTraits::RegisterOne() {
  ...
  if constexpr (details::ContainPmrAlloc<Cmpt>) {
    static_assert(
        details::DefaultCtorWithAlloc<Cmpt>,
        "alloc-awared <Cmpt> must be default-constructible with alloc");
    static_assert(details::CopyCtorWithAlloc<Cmpt>,
                  "alloc-awared <Cmpt> must be copy-constructible with alloc");
    static_assert(details::MoveCtorWithAlloc<Cmpt>,
                  "alloc-awared <Cmpt> must be move-constructible with alloc");
  } else {
