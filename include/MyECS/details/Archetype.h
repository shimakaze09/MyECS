...
class Archetype {
 public:
  ...
  Cmpt* At(std::size_t idx) const {
    return reinterpret_cast<Cmpt*>(At(TypeID_of<Cmpt>, idx));
  }
 ...
 private:
  ...
  std::unordered_map<TypeID, std::size_t>
      type2offset;  // TypeID to offset in chunk (include Entity)
