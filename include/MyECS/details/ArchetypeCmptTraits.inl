...
inline const std::function<void(void*, void*)>*
ArchetypeCmptTraits::GetCopyConstruct(TypeID type) const {
  for (const auto& [t, copy_ctor] : copy_constructors) {
    if (t == type) return &copy_ctor;
  }
  return nullptr;
}

inline const std::function<void(void*, void*)>*
ArchetypeCmptTraits::GetMoveConstruct(TypeID type) const {
  for (const auto& [t, move_ctor] : move_constructors) {
    if (t == type) return &move_ctor;
  }
  return nullptr;
}

inline const std::function<void(void*, void*)>*
ArchetypeCmptTraits::GetMoveAssign(TypeID type) const {
  for (const auto& [t, move_assign] : move_constructors) {
    if (t == type) return &move_assign;
  }
  return nullptr;
}

inline const std::function<void(void*)>* ArchetypeCmptTraits::GetDestruct(
    TypeID type) const {
  for (const auto& [t, dtor] : destructors) {
    if (t == type) return &dtor;
  }
  return nullptr;
}
...
void ArchetypeCmptTraits::Register() {
  ...
  if constexpr (!std::is_trivially_destructible_v<Cmpt>) {
    destructors.emplace_back(
        type, [](void* cmpt) { static_cast<Cmpt*>(cmpt)->~Cmpt(); });
  }
...
inline void ArchetypeCmptTraits::Deregister(TypeID type) noexcept {
  ...
  for (auto iter = copy_constructors.begin(); iter != copy_constructors.end();
       ++iter) {
    const auto& [t, e] = *iter;
    if (t == type) {
  ...
  for (auto iter = move_constructors.begin(); iter != move_constructors.end();
       ++iter) {
    const auto& [t, e] = *iter;
    if (t == type) {
  ...
  for (auto iter = move_assignments.begin(); iter != move_assignments.end();
       ++iter) {
    const auto& [t, e] = *iter;
    if (t == type) {
