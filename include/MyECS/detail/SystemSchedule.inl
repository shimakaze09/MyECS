...
SystemSchedule& SystemSchedule::Regist(Func&& func) {
  if constexpr (std::is_member_function_pointer_v<Func>) {
    Regist(detail::SystemSchedule_::GenSystem<Func>::run(
...
struct Schedule<TypeList<Cmpts...>> {
  ...
  static void Regist(
    ...
    if constexpr (std::is_const_v<Cmpt>) {
      ...
      id2rw[My::TypeID<RawCmpt>].readers.push_back(system);
    } else {
      assert(id2rw[My::TypeID<Cmpt>].writer == nullptr &&
             "two component write same component");
      id2rw[My::TypeID<Cmpt>].writer = system;
    }
  }
};
}  // namespace My::detail::SystemSchedule_
...
struct GenSystemHelper<Cmpt, TypeList<Cmpts...>> {
  ...
  static auto run(Func&& func) noexcept {
    if constexpr (FuncTraits<std::decay_t<Func>>::is_const) {
      return
          [func = std::forward<Func>(func)](const Cmpt* cmpt, Cmpts... cmpts) {
            (cmpt->*func)(cmpts...);
          };
    } else {
