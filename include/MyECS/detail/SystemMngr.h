...
class SystemMngr {
 ...
 private:
  ...
  std::map<ScheduleType, std::vector<std::function<void(SystemSchedule*)>>>
      type2StaticScheduleFuncs;
  std::vector<std::function<void(std::map<ScheduleType, SystemSchedule*>&)>>
      dynamicScheduleFuncs;
};
}  // namespace My
