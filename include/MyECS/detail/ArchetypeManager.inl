...
namespace My::detail::ArchetypeManager_ {
...
const std::tuple<Cmpts*...> ArchetypeManager::EntityAttach(EntityBase* e) {
  ...
  for (auto cmptHash : srcID) {
    ...
    ComponentManager::Instance().MoveConstruct(cmptHash, dstCmpt, srcCmpt);
  }
...
void ArchetypeManager::EntityDetach(EntityBase* e) {
  ...
  for (auto cmptHash : srcID) {
    ...
    if (dstID.IsContain(cmptHash)) {
      ...
      ComponentManager::Instance().Destruct(cmptHash, srcCmpt);
  }
...
void ArchetypeManager::GenTaskflow(tf::Taskflow& taskflow, Sys&& sys) {
  return detail::ArchetypeManager_::GenTaskflow<FuncTraits_ArgList<Sys>>::run(
