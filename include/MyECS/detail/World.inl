...
void World::RunEntityJob(Func&& func, bool isParallel, ArchetypeFilter filter,
  ...
  SystemFunc sys{std::forward<Func>(func),    "",
                 ...
                 std::move(singletonLocator), std::move(isParallel)};
  Run(&sys);
}
...
template <typename Func>
void World::RunJob(Func&& func, SingletonLocator singletonLocator) {
  SystemFunc sys{std::forward<Func>(func), "", std::move(singletonLocator)};
  Run(&sys);
}
}  // namespace My::MyECS
