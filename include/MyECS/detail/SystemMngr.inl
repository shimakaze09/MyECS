...
void SystemMngr::RegisterOne() {
  static_assert(std::is_base_of_v<System, DerivedSystem> &&
                    std::is_constructible_v<DerivedSystem, World*, std::string>,
                "<DerivedSystem> must be derived form System and constructible "
                "with World* and std::string");
  auto derivedSystem = new DerivedSystem{
      world, std::string{nameof::nameof_type<DerivedSystem>()}};
