...
EntityMngr::AttachWithoutInit(Entity e) {
  ...
  if (dstArchetype == srcArchetype)
    return {std::array<bool, sizeof...(Cmpts)>{false},
            {srcArchetype->At<Cmpts>(srcIdxInArchetype)...}};
  ...
  return {{srcArchetype->GetCmptTypeSet().IsNotContain<Cmpts>()...},
          {dstArchetype->At<Cmpts>(dstIdxInArchetype)...}};
}
...
const std::tuple<Cmpts*...> EntityMngr::Attach(Entity e) {
  ...
  return {(std::get<Find_v<CmptList, Cmpts>>(success)
