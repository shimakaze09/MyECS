...
class CmptTraits {
 public:
  ...
  const std::pmr::unordered_map<TypeID, std::string_view>& GetNames()
      const noexcept;
  const std::pmr::unordered_map<TypeID, std::size_t>& GetSizeofs()
      const noexcept;
  const std::pmr::unordered_map<TypeID, std::size_t>& GetAlignments()
      const noexcept;
  const std::pmr::unordered_map<
      TypeID, std::function<void(void*, std::pmr::memory_resource*)>>&
  GetDefaultConstructors() const noexcept;
  const std::pmr::unordered_map<
      TypeID,
      std::function<void(void*, const void*, std::pmr::memory_resource*)>>&
  GetCopyConstructors() const noexcept;
  const std::pmr::unordered_map<
      TypeID, std::function<void(void*, void*, std::pmr::memory_resource*)>>&
  GetMoveConstructors() const noexcept;
  const std::pmr::unordered_map<TypeID, std::function<void(void*, void*)>>&
  GetMoveAssignments() const noexcept;
  const std::pmr::unordered_map<TypeID, std::function<void(void*)>>&
  GetDestructors() const noexcept;
