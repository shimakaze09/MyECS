...
struct CmptTraits::Impl {
  ...
  Impl(const Impl& other, std::pmr::unsynchronized_pool_resource* rsrc)
    ...
    for (const auto& [id, name] : other.names) {
      char* buffer = (char*)rsrc->allocate((name.size() + 1) * sizeof(char),
                                           alignof(char));
  ...
  std::pmr::unordered_map<TypeID, std::function<void(void*, void*)>>
...
const std::pmr::unordered_set<TypeID>& CmptTraits::GetTrivials()
    const noexcept {
  return impl->trivials;
}

const std::pmr::unordered_map<TypeID, std::string_view>& CmptTraits::GetNames()
    const noexcept {
  return impl->names;
}

const std::pmr::unordered_map<TypeID, std::size_t>& CmptTraits::GetSizeofs()
    const noexcept {
  return impl->sizeofs;
}

const std::pmr::unordered_map<TypeID, std::size_t>& CmptTraits::GetAlignments()
    const noexcept {
...
const std::pmr::unordered_map<
    TypeID, std::function<void(void*, std::pmr::memory_resource*)>>&
CmptTraits::GetDefaultConstructors() const noexcept {
...
const std::pmr::unordered_map<
    TypeID,
    std::function<void(void*, const void*, std::pmr::memory_resource*)>>&
CmptTraits::GetCopyConstructors() const noexcept {
  return impl->copy_constructors;
}

const std::pmr::unordered_map<
    TypeID, std::function<void(void*, void*, std::pmr::memory_resource*)>>&
CmptTraits::GetMoveConstructors() const noexcept {
  return impl->move_constructors;
}

const std::pmr::unordered_map<TypeID, std::function<void(void*, void*)>>&
CmptTraits::GetMoveAssignments() const noexcept {
  return impl->move_assignments;
}

const std::pmr::unordered_map<TypeID, std::function<void(void*)>>&
CmptTraits::GetDestructors() const noexcept {
...
CmptTraits& CmptTraits::RegisterName(Type type) {
  ...
  auto* buffer = (char*)impl->rsrc->allocate(
  ...
  impl->names.emplace(type.GetID(),
                      std::string_view{buffer, type.GetName().size()});
