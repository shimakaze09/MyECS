...
std::size_t SystemTraits::GetID(std::string_view name) const {
  auto target = name2id.find(name);
  return target == name2id.end() ? static_cast<std::size_t>(-1)
                                 : target->second;
}

void SystemTraits::RegisterOnCreate(std::size_t ID,
                                    std::function<OnCreate> func) {
  assert(IsRegistered(ID));
  createMap[ID] = std::move(func);
}

void SystemTraits::RegisterOnActivate(std::size_t ID,
                                      std::function<OnActivate> func) {
  assert(IsRegistered(ID));
  activateMap[ID] = std::move(func);
}

void SystemTraits::RegisterOnUpdate(std::size_t ID,
                                    std::function<OnUpdate> func) {
  assert(IsRegistered(ID));
  updateMap[ID] = std::move(func);
}

void SystemTraits::RegisterOnDeactivate(std::size_t ID,
                                        std::function<OnDeactivate> func) {
  assert(IsRegistered(ID));
  deactivateMap[ID] = std::move(func);
}

void SystemTraits::RegisterOnDestroy(std::size_t ID,
                                     std::function<OnDestroy> func) {
  assert(IsRegistered(ID));
