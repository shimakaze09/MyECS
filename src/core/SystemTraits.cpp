...
SystemTraits::SystemTraits(const SystemTraits& other,
...
Name SystemTraits::Register(std::string_view name) {
  ...
  if (target != impl->names.end()) return {target->second, id};

  auto* buffer = (char*)impl->rsrc->allocate((name.size() + 1) * sizeof(char),
                                             alignof(char));
  std::memcpy(buffer, name.data(), name.size() * sizeof(char));
  buffer[name.size()] = 0;
...
void SystemTraits::Unregister(NameID ID) {
  auto target = impl->names.find(ID);
  if (target == impl->names.end()) return;
...
const std::pmr::unordered_map<NameID, std::string_view>&
SystemTraits::GetNames() const noexcept {
  return impl->names;
}
...
void SystemTraits::RegisterOnActivate(NameID ID,
                                      std::function<OnActivate> func) {
  assert(IsRegistered(ID));
...
void SystemTraits::RegisterOnDeactivate(NameID ID,
                                        std::function<OnDeactivate> func) {
  assert(IsRegistered(ID));
