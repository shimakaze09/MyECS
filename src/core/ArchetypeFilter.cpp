...
std::size_t ArchetypeFilter::GetValue() const noexcept {
  ...
  for (const auto& type : all) rst = hash_combine(rst, type.GetValue());
  for (const auto& type : any) rst = hash_combine(rst, type.GetValue());
  for (const auto& type : none) rst = hash_combine(rst, type.GetValue());
  return rst;
}
