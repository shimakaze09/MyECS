...
vector<Schedule::NoneGroup> Schedule::GenSortNoneGroup(
    SysFuncGraph writerGraph, const vector<SystemFunc*>& preReaders,
  ...
  if (!preReaders.empty()) {
    ...
    preGroup.allTypes =
        SetUnion(preReaders.front()->query.Filter().AllCmptTypes(),
                 preReaders.front()->query.Filter().AnyCmptTypes());
    preGroup.allTypes = SetUnion(
        preGroup.allTypes, preReaders.front()->query.Locator().CmptTypes());
    preGroup.noneTypes = preReaders.front()->query.Filter().NoneCmptTypes();
    for (size_t i = 1; i < preReaders.size(); i++) {
      auto allTypes = SetUnion(preReaders[i]->query.Filter().AllCmptTypes(),
                               preReaders[i]->query.Filter().AnyCmptTypes());
      allTypes = SetUnion(preGroup.allTypes,
                          preReaders[i]->query.Locator().CmptTypes());
      preGroup.allTypes = SetIntersection(preGroup.allTypes, allTypes);
      preGroup.noneTypes = SetIntersection(
          preGroup.noneTypes, preReaders[i]->query.Filter().NoneCmptTypes());
    }

    writerGraph.AddVertex(preReaders.front());
    for (auto writer : writers) {
      auto allTypes = SetUnion(writer->query.Filter().AllCmptTypes(),
                               writer->query.Filter().AnyCmptTypes());
      allTypes =
          SetUnion(preGroup.allTypes, writer->query.Locator().CmptTypes());
      if (SetIntersection(preGroup.allTypes,
                          writer->query.Filter().NoneCmptTypes())
              .empty() &&
  ...
  if (!postReaders.empty()) {
    ...
    postGroup.allTypes =
        SetUnion(postReaders.front()->query.Filter().AllCmptTypes(),
                 postReaders.front()->query.Filter().AnyCmptTypes());
    postGroup.allTypes = SetUnion(
        postGroup.allTypes, postReaders.front()->query.Locator().CmptTypes());
    postGroup.noneTypes = postReaders.front()->query.Filter().NoneCmptTypes();
    for (size_t i = 1; i < postReaders.size(); i++) {
      auto allTypes = SetUnion(postReaders[i]->query.Filter().AllCmptTypes(),
                               postReaders[i]->query.Filter().AnyCmptTypes());
      allTypes = SetUnion(postGroup.allTypes,
                          postReaders[i]->query.Locator().CmptTypes());
      postGroup.allTypes = SetIntersection(postGroup.allTypes, allTypes);
      postGroup.noneTypes = SetIntersection(
          postGroup.noneTypes, postReaders[i]->query.Filter().NoneCmptTypes());
    }
    ...
    for (auto writer : writers) {
      auto allTypes = SetUnion(writer->query.Filter().AllCmptTypes(),
                               writer->query.Filter().AnyCmptTypes());
      allTypes =
          SetUnion(postGroup.allTypes, writer->query.Locator().CmptTypes());
      if (SetIntersection(postGroup.allTypes,
                          writer->query.Filter().NoneCmptTypes())
              .empty() &&
  ...
  for (auto func : sortedFuncs) {
    if (!preReaders.empty() && func == preReaders.front())
      ...
      NoneGroup group;
      group.allTypes = SetUnion(func->query.Filter().AllCmptTypes(),
                                func->query.Filter().AnyCmptTypes());
      group.allTypes =
          SetUnion(group.allTypes, func->query.Locator().CmptTypes());
...
SysFuncGraph Schedule::GenSysFuncGraph() const {
  ...
  for (const auto& [type, cmptSysFuncs] : cmptSysFuncsMap) {
    ...
    auto sortedGroup =
        GenSortNoneGroup(subgraph, cmptSysFuns.lastFrameSysFuncs,
                         cmptSysFuns.writeSysFuncs, cmptSysFuns.latestSysFuncs);
