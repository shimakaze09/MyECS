...
Schedule::NoneGroup::NoneGroup(SystemFunc* func) : sysFuncs{func} {
  allTypes = SetUnion(func->query.filter.AllCmptTypes(),
...
void Schedule::SetPrePostEdge(SysFuncGraph& graph,
  ...
  if (!postReaders.empty()) {
    ...
    for (size_t i = 1; i < postReaders.size(); i++) {
      ...
      postGroup.noneTypes =
          SetIntersection(postGroup.noneTypes, group.noneTypes);
    }
...
vector<Schedule::NoneGroup> Schedule::GenSortNoneGroup(
    SysFuncGraph graph, const std::vector<SystemFunc*>& preReaders,
    const std::vector<SystemFunc*>& writers,
    const std::vector<SystemFunc*>& postReaders) {
...
SysFuncGraph Schedule::GenSysFuncGraph() const {
  ...
  for (const auto& [type, cmptSysFuncs] : cmptSysFuncsMap) {
    ...
    auto sortedGroup = GenSortNoneGroup(graph, cmptSysFuncs.lastFrameSysFuncs,
