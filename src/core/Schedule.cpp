...
struct NoneGroup {
  ...
  NoneGroup& operator+=(const NoneGroup& y) {
    AccessTypeIDSet rst_allTypes, rst_anyTypes, rst_randomTypes, union_anyTypes,
        diff_allTypes;
    set<TypeID> rst_noneTypes;
    set<SystemFunc*> rst_sysFuncs;
    std::set_intersection(allTypes.begin(), allTypes.end(), y.allTypes.begin(),
                          y.allTypes.end(),
                          std::insert_iterator<AccessTypeIDSet>(
                              rst_allTypes, rst_allTypes.begin()));
    std::set_union(anyTypes.begin(), anyTypes.end(), y.anyTypes.begin(),
                   y.anyTypes.end(),
                   std::insert_iterator<AccessTypeIDSet>(
                       union_anyTypes, union_anyTypes.begin()));
    std::set_symmetric_difference(allTypes.begin(), allTypes.end(),
                                  y.allTypes.begin(), y.allTypes.end(),
                                  std::insert_iterator<AccessTypeIDSet>(
                                      diff_allTypes, diff_allTypes.begin()));
    std::set_union(union_anyTypes.begin(), union_anyTypes.end(),
                   diff_allTypes.begin(), diff_allTypes.end(),
                   std::insert_iterator<AccessTypeIDSet>(rst_anyTypes,
                                                         rst_anyTypes.begin()));
    std::set_union(randomTypes.begin(), randomTypes.end(),
                   y.randomTypes.begin(), y.randomTypes.end(),
                   std::insert_iterator<AccessTypeIDSet>(
                       rst_randomTypes, rst_randomTypes.begin()));
    std::set_intersection(noneTypes.begin(), noneTypes.end(),
                          y.noneTypes.begin(), y.noneTypes.end(),
                          std::insert_iterator<set<TypeID>>(
                              rst_noneTypes, rst_noneTypes.begin()));
    std::set_union(sysFuncs.begin(), sysFuncs.end(), y.sysFuncs.begin(),
                   y.sysFuncs.end(),
                   std::insert_iterator<set<SystemFunc*>>(
                       rst_sysFuncs, rst_sysFuncs.begin()));
...
struct Compiler {
  ...
  static vector<NoneGroup> GenSortNoneGroup(
      const SysFuncGraph& graph, const Schedule::CmptSysFuncs& cmptFuncs,
      const unordered_map<SystemFunc*, NoneGroup>& gMap) {
    const auto& preReaders = cmptFuncs.lastFrameSysFuncs;
    const auto& writers = cmptFuncs.writeSysFuncs;
    const auto& postReaders = cmptFuncs.latestSysFuncs;
...
unordered_map<TypeID, Schedule::CmptSysFuncs> Schedule::GenCmptSysFuncsMap()
    const {
