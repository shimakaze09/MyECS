...
MyGraphviz::Graph World::GenUpdateFrameGraph() const {
  ...
  auto& subgraph_lastframe_all =
      subgraph_lastframe.GenSubgraph("LastFrame All Edges");
  ...
  subgraph_lastframe_all.RegisterGraphEdgeAttr("style", "dashed")
      .RegisterGraphEdgeAttr("arrowhead", "box");
  subgraph_write_all.RegisterGraphEdgeAttr("style", "dashed")
      .RegisterGraphEdgeAttr("arrowhead", "box");
  subgraph_latest_all.RegisterGraphEdgeAttr("style", "dashed")
      .RegisterGraphEdgeAttr("arrowhead", "box");

  subgraph_basic_any.RegisterGraphEdgeAttr("style", "dashed")
      .RegisterGraphEdgeAttr("arrowhead", "diamond");
  subgraph_lastframe_any.RegisterGraphEdgeAttr("style", "dashed")
      .RegisterGraphEdgeAttr("arrowhead", "diamond");
  subgraph_write_any.RegisterGraphEdgeAttr("style", "dashed")
      .RegisterGraphEdgeAttr("arrowhead", "diamond");
  subgraph_latest_any.RegisterGraphEdgeAttr("style", "dashed")
      .RegisterGraphEdgeAttr("arrowhead", "diamond");
  ...
  for (const auto& [hash, sysFunc] : schedule.sysFuncs) {
    ...
    for (const auto& cmptType : sysFunc->entityQuery.filter.all) {
      ...
      switch (cmptType.GetAccessMode()) {
        case AccessMode::LAST_FRAME:
          edgeIdx = registry.RegisterEdge(cmptType2idx.at(cmptType), sysIdx);
          (isChunk ? subgraph_lastframe_all : subgraph_basic_all)
              .AddEdge(edgeIdx);
          break;
        case AccessMode::WRITE:
          edgeIdx = registry.RegisterEdge(sysIdx, cmptType2idx.at(cmptType));
