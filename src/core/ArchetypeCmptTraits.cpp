...
void ArchetypeCmptTraits::CmptTrait::CopyConstruct(
    void* dst, const void* src, std::pmr::memory_resource* rsrc) const {
  if (copy_ctor)
...
void ArchetypeCmptTraits::CmptTrait::MoveConstruct(
    void* dst, void* src, std::pmr::memory_resource* rsrc) const {
  if (move_ctor)
...
ArchetypeCmptTraits::CmptTrait& ArchetypeCmptTraits::GetTrait(
    TypeID ID) noexcept {
...
void ArchetypeCmptTraits::Register(const CmptTraits& rtdct, TypeID type) {
  ...
  if (size_target == rtdct.GetSizeofs().end())
    throw std::logic_error(
        "ArchetypeCmptTraits::Register: RTDCmptTrait hasn't registered "
        "<TypeID>");
  ...
  CmptTrait trait{.trivial = rtdct.IsTrivial(type),
  ...
  std::size_t idx =
      static_cast<std::size_t>(std::distance(types.begin(), iter));
  if (success)
