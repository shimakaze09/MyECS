...
class Archetype {
 public:
  ...
  CmptAccessPtr WriteAt(TypeID type, EntityAddress addr) const {
    return At({type, AccessMode::WRITE}, addr);
  }
  CmptAccessPtr ReadAt(TypeID type, EntityAddress addr) const {
    return At({type, AccessMode::LATEST}, addr);
  }

  // nullptr if not contains
  template <typename Cmpt>
  Cmpt* WriteAt(EntityAddress addr) const {
    return WriteAt(TypeID_of<Cmpt>, addr)
        .template As<Cmpt, AccessMode::WRITE>();
  }
  template <typename Cmpt>
  const Cmpt* ReadAt(EntityAddress addr) const {
    return ReadAt(TypeID_of<Cmpt>, addr)
        .template As<Cmpt, AccessMode::LATEST>();
  }

  // no Entity
  std::vector<CmptAccessPtr> Components(EntityAddress addr,
                                        AccessMode mode) const;
  std::vector<CmptAccessPtr> WriteComponents(EntityAddress addr) const {
    return Components(addr, AccessMode::WRITE);
  }
  std::vector<CmptAccessPtr> ReadComponents(EntityAddress addr) const {
    return Components(addr, AccessMode::LATEST);
  }
  ...
  std::span<Chunk*> GetChunks() noexcept {
    return {chunks.data(), chunks.size()};
  }
  std::span<std::size_t> GetOffsets() noexcept {
 ...
 private:
  ...
  small_flat_set<std::size_t, 16, std::greater<std::size_t>>
      nonFullChunks;  // big -> small, use small first
