...
class MyResource : public std::pmr::unsynchronized_pool_resource {
 ...
 protected:
  virtual void* do_allocate(const size_t _Bytes, const size_t _Align) override {
    void* _Ptr =
        this->unsynchronized_pool_resource::do_allocate(_Bytes, _Align);
    if (verbose)
      std::cout << "  allocate @" << _Ptr << " (" << _Bytes << ", " << _Align
                << ")" << std::endl;
    buffers.insert(_Ptr);
    return _Ptr;
  }

  virtual void do_deallocate(void* const _Ptr, const size_t _Bytes,
                             const size_t _Align) override {
    if (verbose)
      std::cout << "deallocate @" << _Ptr << " (" << _Bytes << ", " << _Align
                << ")" << std::endl;
    buffers.erase(_Ptr);
